...
\begin{flushleft}
Given, \(X\) is a continuous random variable in range [0,100].\newline

The probability density function is \(Pr(X=x)\)
\end{flushleft}
\begin{align}
    Pr(X=x) &= f(x) \;\;\forall\; x \in [0,100]\\
    &= 0.01 \;\;\forall\; x \in [0,100]\\\nonumber
\end{align}

\begin{flushleft}
Mean of the random variable X is \(E(X)\)
\end{flushleft}

\begin{align}
    E(X) &= \int x\,Pr(X=x)\,\mathrm{d}x\\\nonumber\\
